...
import { foldersSelector } from './mail';
import { DefaultRootState } from 'react-redux';
import { MailboxFolder } from '../mail';
  ...
  (folders, aliases, elementsByFolder, elementsByAlias) => {
    ...
    Object.keys(elementsByFolder).forEach((folderId: string) => {
      const folder = folders.find(
        (folderItem: MailboxFolder) =>
          folderItem.folderId === parseInt(folderId, 10),
      );
      if (!folder) {
        return;
      }
